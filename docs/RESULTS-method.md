...
M0 remains proven. M1 is not satisfied: no clean game has cleared a level. The next required evidence is a fresh v6 BP35 diagnostic without a concurrent run, followed by a held-out run only if its predicted process signature appears. No claim of generalization or ~99% reproduction is currently supported.
