...
V8 produced that signature and a development-game clear, so it remains frozen through the administratively interrupted vc33 continuation. The remaining candidates are predictive-rule fidelity and decision-relevant probe ordering first, phase control second, then terminal-flag/BFS coupling once a clean run provides an evidenced goal predicate. Adding any of them before completing the censored held-out trajectory would destroy attribution.

## Status and Uncertainty

M0 remains proven, and the contaminated development gate now passes: v8 cleared BP35 level 0 within its 21-action boundary while maintaining exact retrospective replay. M1 is still not satisfied because no clean game has cleared a level. The first preselected vc33 attempt was administratively censored after four actions by the Claude subscription limit. The next required evidence is an unchanged continuation from that exact state, restoring `4/4` replay before action 5 and preserving the absolute 21-action boundary. The combined result must be labeled resumed-after-quota rather than pristine; a second unopened game remains necessary for clean held-out evidence. No claim of generalization or ~99% reproduction is currently supported.
