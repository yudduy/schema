# Physicist Method v9 — Local Repair and Matched Transfer
...
Immediately before `commit_actions`, forward-simulate the proposed queue in the executable model and verify that its predicted effects support the stated reason; for every click, mechanically verify that `(x, y)` belongs to the intended current-grid component rather than trusting coordinates copied into notes.
